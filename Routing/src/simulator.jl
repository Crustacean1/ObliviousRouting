...
function create_impr_valiant(graph, src, dst)
	...
	if(count_ones(xor(src,intermediary[1])) + count_ones(xor(intermediary[1],dst)) < count_ones(xor(src,inv[1])) + count_ones(xor(inv[1],dst)))
		...
		println("Selecting the invrse: $inv $intermediary $src $dst")
...
graph = hypergrid(2,10)
demands = permutation(size(graph,1))
demands = map(x -> xor(x-1,size(graph,1)-1) + 1, 1:size(graph,1))
println("Permutation: ",demands)

trace = network_simulation(graph, demands, create_impr_valiant)
println("Trace")
display(trace)
println("Dilation")
